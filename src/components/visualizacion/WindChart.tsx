...
export default function WindChart({ forecastData }: { forecastData: any }) {
  ...
  const allHours = forecastDays?.flatMap((d: any) => d.hours) ?? [];
  const times = allHours.slice(0, 72).map((h: any) => fmtHour(h.time));
  const wind10m = allHours.slice(0, 72).map((h: any) => h.windSpeed10mKmh);
  const wind2m = allHours.slice(0, 72).map((h: any) => h.windSpeed2mKmh);

  const dailySummaries = forecastDays?.map((d: any) => d.dailySummary) ?? [];
  const dayLabels = forecastDays?.map((d: any) => new Date(d.date).toLocaleDateString('es-ES', { weekday: 'short', day: 'numeric' })) ?? [];
  const windMean = dailySummaries.map((s: any) => s.windMeanKmh);

  const hasHourly = times.length > 0 && wind10m.some((v: any) => v != null);
  const hasDaily = dayLabels.length > 0 && windMean.some((v: any) => v != null);
    ...
    <section className="surface-card-strong rounded-[28px] p-5 sm:p-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-black text-slate-950">Velocidad y ráfagas</h2>
